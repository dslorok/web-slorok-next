...
export default function BumdesSection() {
  ...
  const loadNews = async () => {
    setLoading(true)
    try {
      const newsData = await fetchNewsData('bumdes')
      setNews(newsData)
    } catch (error) {
  ...
  const handleDelete = async (newsItem: NewsItem, e: React.MouseEvent) => {
    ...
    if (confirm(`Hapus berita "${newsItem.title}"?`)) {
      try {
        ...
        await deleteNews(newsItem.id.toString(), 'bumdes')
  ...
  const handleSaveNews = async (formData: NewsFormData) => {
    try {
      if (editingNews) {
        ...
        await updateNews(editingNews.id.toString(), formData, 'bumdes')
        console.log('News updated successfully')
      } else {
        // Add new news
        await addNews(formData, 'bumdes')
      ...
      <section className={`py-16 bg-gray-50 dark:bg-gray-800`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-medium text-green-800 flex items-center">
                    ...
                    Mode Admin - Kelola Galeri Kegiatan BUMDes
                  </h3>
          ...
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-primary-600 dark:text-primary-400 mb-4">Galeri Kegiatan Badan Usaha Milik Desa</h2>
